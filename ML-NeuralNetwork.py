...
n_classes = 2
batch_size = 32
total_batch = int(1600000/batch_size)
hm_epochs = 10

x = tf.placeholder('float')
y = tf.placeholder('float')

hidden_layer1 = {'f_fum': n_nodes_hl1,
					'weights': tf.Variable(tf.random_normal([2638, n_nodes_hl1])),
					'bias': tf.Variable(tf.random_normal([n_nodes_hl1]))}
hidden_layer2 = {'f_fum': n_nodes_hl2,
...
#we're going to save our model in the form of checkpoints as time goes on
saver = tf.train.Saver()
tf_log = 'tf.log'

def train_neural_network(x):
	prediction = neuralNetworkLayers(x)
	cost = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(logits = prediction, labels = y))
	optimizer = tf.train.AdamOptimizer(learning_rate = 0.001).minimize(cost)
	with tf.Session() as sess:
		sess.run(tf.global_variables_initializer())
		try:
			epoch = int(open(tf.log, 'r').read().split('\n')[-2]) + 1
			print('Start', epoch)
		except Exception as e:
			epoch = 1

		while epoch <= hm_epochs:
			#As the epoch begins, if we're not starting at the first one,
			# then we're going to load in the checkpoint file
			if epoch != 1:
				saver.restore(sess, 'model.ckpt')

			epoch_loss = 1
			with open('lexiconTest.pickle', 'rb') as f:
